...
class Paymentfive extends Base
{
    private  $key = 'fs8gzx6udeakyiigqcvemmc473ql5saj';
    private  $url = 'https://api.victory-pay.com/payweb/recharge';
    private  $pay_memberid = '8000413';
    ...
    public function Map($data,$type) {
        ...
        Db::name('pay_info')->insert(['text'=>'发起充值1：'.$json,'time'=>date('Y-m-d H:i:s',time())]);
    ...
    public function RechargeMoneyCallBack($data){
        ...
        $json = json_encode($data);
        // }
        Db::name('pay_info')->insert(['text'=>'充值异步1：'.$json,'time'=>date('Y-m-d H:i:s',time())]);
    ...
    public function RechangeCallBack(Request $request)
    {
        ...
        Db::name('pay_info')->insert(['text'=>'充值回调异步1：'.$json,'time'=>date('Y-m-d H:i:s',time())]);
        ...
        if(!$res){
            Db::name('pay_info')->insert(['text'=>'充值回调异步1：未找到该订单！','time'=>date('Y-m-d H:i:s',time())]);
            echo 'success';
            exit();
        }
        
        if($res['status'] != 0){
            Db::name('pay_info')->insert(['text'=>'充值回调异步1：该订单已处理','time'=>date('Y-m-d H:i:s',time())]);
            echo 'success';
            exit();
        }
        ...
        try {
            ...
            Db::name('pay_info')->insert(['text'=>'充值回调异步1：'.$data['data']['mer_order_num'].'完成。','time'=>date('Y-m-d H:i:s',time())]);
            echo 'success';
            //return"OK";
            //exit();
            ...
            Db::rollback();
            Db::name('pay_info')->insert(['text'=>'充值回调异步1：'.$data['data']['mer_order_num'].$exception->getMessage(),'time'=>date('Y-m-d H:i:s',time())]);
            echo 'success';
            //return 'OK';
            //return"OK";
            //exit();
    ...
    public function PaymentCallBack(Request $request)
    {
        ...
        Db::name('pay_info')->insert(['text'=>'提现回调异步1：'.$json,'time'=>date('Y-m-d H:i:s',time())]);
        //exit();
        
        $withdraw_info = Db::name('member_bm_withdraw')->where("order_id = '".$data['data']['mer_order_num']."'")->find();
        if(empty($withdraw_info)){
            Db::name('pay_info')->insert(['text'=>'提现回调异步1：未找到该订单！','time'=>date('Y-m-d H:i:s',time())]);
            echo 'success';
            exit();
        }
        
        if($withdraw_info['status'] != 0 && $withdraw_info['status'] != 3){
            Db::name('pay_info')->insert(['text'=>'提现回调异步1：该订单已处理','time'=>date('Y-m-d H:i:s',time())]);
            echo 'success';
            exit();
        }
        ...
        try {
            ...
            if($data['code'] == 200){
                ...
                Db::name('member_balance_log')->insert([
                    ...
                    'change_money' => $data['data']['price'],
                    'after_money' => $member_info['balance'] + $data['data']['price'],
                    'message' => '提现失败退回'.$data['data']['price'],
                    'message_e' => 'Withdrawal failed and returned '.$data['data']['price'],
                    'type' => 2,
                    'bo_time' => getIndaiTime(time()),
                    'status' => 93
                ]);
                ...
                Db::name('member_list')->where('id', $withdraw_info['uid'])->update([
                    'balance' => Db::raw('balance +'.$data['data']['price']),
            ...
            Db::name('pay_info')->insert(['text'=>'提现回调异步1：'.$data['data']['mer_order_num'].'完成。','time'=>date('Y-m-d H:i:s',time())]);
            echo 'success';
            //return"OK";
            //return json(['code' => 1,'msg' => getErrorInfo(SUCCESS)]);
        } catch (Exception $exception) {
            Db::rollback();
            Db::name('pay_info')->insert(['text'=>'提现回调异步1：'.$data['data']['mer_order_num'].$exception->getMessage(),'time'=>date('Y-m-d H:i:s',time())]);
            echo 'success';
            //return"OK";
            //return json(['code' => 2,'msg' => getErrorInfo(ADD_FAIL).$exception->getMessage()]);
        }
