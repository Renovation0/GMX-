...
{
    public $table="zm_member_list";
    
    
    
    //会员增加矿机
    public function updateMachineNnum($where,$num)
    {
        return $this->where($where)->setInc('eff_machine',$num);
    }
...
/*         $beginYesterday = mktime(0,0,0,date('m'),date('d')-1,date('Y'));
        ...
        //总充值金额
        $member_ykb = $this->sum('rechange_limit_total');
        
        //激活币总额
        $member_jhb = $this->sum('balance_total');
        //$member_jhb = Db::name('member_balance_log')->where('type = 2 AND status = 91')->sum('change_money');
        ...
        $data = [
            ...
            'member_jhb' =>  abs($member_jhb),//$member_jhb,
        ];
